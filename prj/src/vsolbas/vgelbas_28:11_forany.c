...
//		disp_basis_by_ron(&(basis_vector[i]));
		...
		if (basis_vector[i].total == 0) {
			
			basis2->A = basis_vector[i].A;
			basis2->B = basis_vector[i].B;
			basis2->ind = basis_vector[i].ind;
			basis2->nv = basis_vector[i].nv;
			basis2->nb = basis_vector[i].nb;
			basis2->size = basis_vector[i].size;
			basis2->total = basis_vector[i].total;
			
			st = copy_win(&win_dil, &(basis_vector[i].win));
...
LL diferenca_aprox2 (trv_struc * base1, trv_struc * base2) {
	int ind1, ind2, k, i, l;
	LL diferenca = 0;
	...
	for(i = 0; i < base1->total; i++){
		ind1 = base1->ind[i];
		int cnt = 0;
		for (k=0; k<base1->nv; k++) 
			if ((base1->A[ind1+k/NB] & mask[k%NB]) ==  0) cnt++;
		if (cnt >=61) return oo;
		diferenca -= (1LL<<cnt);
	}
...
LL diferenca_aprox (trv_struc * base1, trv_struc * base2) {
	/* calcula base2 - base1 */
	//igual = diferenca_aprox (&base_Y, base); base_Y dilate after erosion
	win_struc sup_win;
	int ind1, ind2, k, l, n4bytes, cnt1, cnt2, flag, a, b, i, j;
	int vis[base2->total];
	...
	for(i = 0; i < base1->total; i++){
		...
		for(l = 0; l < base2->total; l++){
			...
			for (k=0; k<base1->nv && flag; k++) {
				a = 0; b = 0;
				if ((base1->A[ind1+k/NB] & mask[k%NB]) !=  0) a = 1;
				if ((base2->A[ind2+k/NB] & mask[k%NB]) !=  0) b = 1;
				
//				printf("a=%d b=%d cnt=%d i=%d l=%d\n", a, b, cnt, i, l);
				if (!a && b) {
					flag = 0;
					break;
				}
...
//				printf("a=%d b=%d cnt=%d i=%d l=%d\n", a, b, cnt, i, l);
			...
			if (flag) {
//				printf("entra papa diferenca %d, i=%d, l=%d\n", cnt1, i+1, l+1);
				vis[l] = 1;
				if (cnt2 >= 62) return oo;
...
//	printf("diferenca 1 = %lld\n", diferenca);
	...
	for(l = 0; l < base2->total; l++) if (!vis[l]) {
		...
		cnt1 = 0;
		for (k=0; k<base2->nv; k++) {
			if ((base2->A[ind2+k/NB] & mask[k%NB]) ==  0) cnt1++;
...
/*=*/ int testa_equacao (trv_struc_list **s, trv_struc * base, 
	...
	diferenca = diferenca_aprox (&base_Y, base);
...
//	printf("diferenca = %d\n", diferenca);
	...
	for (i=0; i<total; i++) { /* num. total de prod. cartesiano */
		...
		if (base1.total) {
			...
			diferenca_low = diferenca_aprox (&base_Y, base);
			//			printf("dif %d\n", igual);
			if (diferenca_low == diferenca) {
				tem = 0;
...
//			printf("diferenca = %lld\n", diferenca);
			...
			// disp_win_by_ron (&C);
			/* verifica se a diferenca é a menor*/
			if (diferenca < diferencaC) {
				if (diferencaC == oo) {
					...
					copy_win (&C, &Lista_C->win);
				}
				diferencaC = diferenca;
				//				printf("%d\n", diferencaC);
			}
			if (diferencaC == 0) break;
		}
		if (comb) free (comb);
		if (diferencaC == 0) break;
	}
...
/*=*/ int paralelo_para_sequencial (trv_struc * base) {
	...
	if (base_identidade (base)) {
		...
		for (i=0; i<topo; i++) {
			if (pilha[i].dil == 1) 
				printf ("dil com diferenca %lld\n", pilha[i].dif);
			else
				printf ("ero com diferenca %lld\n", pilha[i].dif);
	...
	for (i=0; i<2; i++) {
		...
		while (sai) {
			...
			while (solucao) {
				...
				pilha[topo].dif = apt_sai->diferenca;
				pilha[topo++].dil = dil;
