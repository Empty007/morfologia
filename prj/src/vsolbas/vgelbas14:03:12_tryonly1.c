...
/*=*/ int testa_equacao (trv_struc_list **s, trv_struc * base, 
	...
	for (i=0; i<total; i++) { /* num. total de prod. cartesiano */
		...
		printf("Testa lowers\n");
		disp_basis_by_ron (&base1);
		zera_basis_by_ron (&base5);
		free_basis_by_ron (&base5);
		zera_basis_by_ron (&base6);
		free_basis_by_ron (&base6);
		basis_dual_operator (&base1, &base5);
		basis_dual_operator (&upper, &base6);
		printf ("Disp base dual\n");
		//intersection lowers dual
		printf("Intersecao\n");
		disp_basis_by_ron (&base5);
		if (base5.total == 18) {
			
		}
		printf("Dual upper\n");
		//upper dual
		disp_basis_by_ron (&base6);
		
		//diferencia
		//		printf("Diferencia = %d\n", diferenca_basis(&base6, &base5));
