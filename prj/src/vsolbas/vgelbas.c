...
typedef struct {
	trv_struc base_lower;
	trv_struc base_upper;
	int igual;
} trv_interval;

typedef struct interval_list {
	trv_interval interval;
	struct interval_list * prox;
} trv_interval_list;
...
typedef struct saida_lista_search_interval {
	trv_interval_list * list_interval;
	win_struc C;
	struct saida_lista_search_interval * prox;
}output_search_interval;
...
int special_erosion_basis(basis1, str, basis2, lock)
...
win_struc * lock;
{
	...
	for(i = 0; i < points; i++){
		...
		diminui_desmonta (&(basis_vector[i]), lock);
		//		disp_basis_by_ron(&(basis_vector[i]));
		//		check_interval(&(basis_vector[i]));
		if(st == 0){
			fprintf(stderr,"dilate_basis(): copy_basis() failed\n");
	...
	for(i = 1; i < points; i++){
		...
		free_itrv(basis_vector + i);
		if (basis11.A) free(basis11.A);
		if (basis11.B) free(basis11.B);
		if (basis11.total) free(basis11.ind);
	...
	if (basis_vector) free(basis_vector); 
	basis2->A = basis11.A;
	basis2->B = basis11.B;
	...
	for (i=0; i<points; i++) {
		...
		if (base_aux.total == 1) {
			...
			if (!retorno) {
				...
				if (retorno2) {
//					printf("Añade na base\n");
					coloca_na_base (sai, &base_aux);
//					printf("elemento por q trasladado %d\n", i);
...
/*=*/ int gera_base_invariante (trv_struc * Z, aha_struc * aha, 
	...
	for (j=0; j<tam_aha; j++) { /* para cada conjunto H */
		if (aha[j].total == 1) {
			...
			esse[j].nv = aha[j].nv;
			esse[j].nb = aha[j].nb;
			esse[j].total = esse[j].size = aha[j].intervs[0].total;
			...
			for (k=0; k<aha[j].intervs[0].total; k++) {
				esse[j].ind[k] = ind = aha[j].intervs[0].ind[k];
				for (jj=0; jj<aha[j].nb; jj++) {
					esse[j].A[ind+jj] = aha[j].intervs[0].A[ind+jj];
					esse[j].B[ind+jj] = aha[j].intervs[0].B[ind+jj];
				}
			}
			// printf ("Esse %d\n", j);
	...
	for (j=0; j<tam_aha; j++) { /* para cada conjunto H */
		if (aha[j].total == 1) break;  /* primeiro que tem diferente de zero */
	}
...
/*=*/ int upper_bound_by_ron (trv_struc * base_out, trv_struc * base_in, 
	...
	free_basis_by_ron (base_out);
	special_erosion_basis (base_in, str, base_out, lock);
...
//	if (base1->total >= 100 && base2->total >= 100) {
//		disp_basis_by_ron(base1);
//		disp_basis_by_ron(base2);
//	}
	
	for (i=0, tem=1; i<base1->total && tem; i++) { 
		/* para cada intervalo da base1 */
		ind1 = base1->ind[i];
		for (j=0, tem=0; j<base2->total && !tem; j++) { 
...
//			if (base1->total >= 100 && base2->total >= 100) {
//				printf ("entre %d e %d temos tem = %d\n", i, j, tem);
//			}
		}
	}
	return tem;
...
/*=*/ int diminui_base_tamanho (trv_struc * base) { //joga fora os 0's que ficam na base original
	
	int i, j, k, sizewin, tamwin, size_dil, k_1, cntbits, cntb, nb;
	int cntbitsapagados, novotamwin, novosbits, nshifts, section, apagar;
	trv_struc base_sai;
	win_struc win_ero;
	...
	win_ero.wsize = base->win.wsize;
	win_ero.hsize = base->win.hsize;
	
	sizewin = function_size_win (&base->win);
	tamwin = (sizewin+31)/32;
	
	size_dil = win_ero.wsize * win_ero.hsize;
	
	k_1 = (size_dil+31)/32;
	win_ero.data = (int *) xmalloc (sizeof(int)*k_1);
	for (j=0; j<k_1; j++) win_ero.data[j] = base->win.data[j];
	
//	disp_basis_by_ron(base);
	
	cntbits = 0;
	cntbitsapagados = 0;
	int apagarbits[sizewin];
	for (i=0; i<base->win.hsize; i++) {
		for (j=0; j<base->win.wsize; j++) {
			nshifts = (i*base->win.wsize+j)%NB;
			section = (i*base->win.wsize+j)/NB;
			apagar = 0;
			if (base->win.data[section] & mask[nshifts]) {
				apagar = 1;
				for (k = 0; k < base->total; k++) {
					if (base->A[tamwin*k + cntbits/NB] & mask[cntbits%NB]) {
						apagar = 0;
						break;
					}
				}
			...
			if (apagar) {
				win_ero.data[section] &= ~(mask[nshifts]);
				apagarbits[cntbitsapagados] = cntbits-1;
				cntbitsapagados++;
			}
		}
	}
	
	novosbits = cntbits - cntbitsapagados;
	novotamwin = (novosbits+NB1)/NB;
	
	win_ero.size = novosbits;
	
	zera_basis_by_ron (&base_sai);
	base_sai.total = base_sai.size = base->total;
	copy_win (&win_ero, &base_sai.win);
	
	base_sai.nv = novosbits;
	base_sai.nb = (novosbits + NB1)/NB;
	base_sai.ind = (int *) xmalloc (sizeof(int) * base->total);
	base_sai.A = (int *) xmalloc (sizeof(int) * base->total * 
								  base_sai.nb);
	base_sai.B = (int *) xmalloc (sizeof(int) * base->total * 
	...
	for (k = 0; k < base->total*novotamwin; k++) {
		base_sai.A[k] = 0;
		base_sai.B[k] = 0;
	}

	for (k = 0; k < base->total; k++) {
		base_sai.ind[k] = k*base_sai.nb;
		cntb = 0; nb = 0;
		for (i = 0; i < cntbits; i++) {
			if (cntb < cntbitsapagados && i == apagarbits[cntb]) {
				cntb++;
				continue;
			}
			if (base->A[tamwin*k + i/NB] & mask[i%NB])
				base_sai.A[k*novotamwin + nb/NB] |= mask[nb%NB];
			nb++;
		}
	}
...
//	diminui_base_tamanho(&basis1);
	
	disp_basis_by_ron(&basis1);
	
	/* criar intervalo */
	intervalo = (trv_interval *) xmalloc (sizeof (trv_interval));
	copy_basis_by_ron (&basis1, &intervalo->base_lower);
	copy_basis_by_ron (&basis1, &intervalo->base_upper);
	intervalo->igual = 1;
	
	/* Caso crescente 2 */
	paralelo_para_sequencial (intervalo);
...
int otimizar_aha(aha_struc * aha, 
				  int tam_aha, trv_struc * base_invariante) {
	
	int j, k, i, jj, ok, bo, ind, tem;
//	printf("qqqq\n");
	/* gerando os conjuntos esse[j], isto e', S_j */
	for (j=0; j<tam_aha; j++) { /* para cada conjunto H */
		if (aha[j].total >= 2) {
			ok = 0;
			for (k = 0; k < aha[j].total; k++) {
				tem = 0;
//				printf("nv = %d\n", aha[j].nv);
				for (i = 0; i < base_invariante->total; i++) {
					ind = base_invariante->ind[i];
					tem = intervalo_contido (&aha[j].intervs[k].A[0], &aha[j].intervs[k].B[0], 
											 &base_invariante->A[ind], &base_invariante->B[ind], aha[j].nv);
//					printf("i = %d, tem = %d, aha = %d, baseA = %d\n", i, tem, aha[j].intervs[k].A[0], base_invariante->A[ind]);
					if (tem) break;
				}
...
//						bo = 1;
//						for (jj=0; jj<aha[j].nb; jj++) {
//							if (aha[j].intervs[k].A[jj] != aha[i].intervs[0].A[jj]) {
//								bo = 0;
//								break;
//							}
//						}
...
int contar1s(aha_struc * aha, 
			 int tam_aha) {
	
	int j, k, i, jj, number, cnt;
	for (j=0; j<tam_aha; j++) { /* para cada conjunto H */		
		for (k = 0; k < aha[j].total; k++) {
			cnt = 0;
			for (jj=0; jj<aha[j].nb; jj++) {
				number = aha[j].intervs[k].A[jj];
				while (number) {
					if (number & 1) cnt++;
					number >>= 1;
				}
...
/*=*/ int testa_equacao (trv_interval_list **s, trv_struc * base, 
						 mat_str * str, trv_struc * upper) {
	trv_struc * quebrada, base_Y;
//	trv_struc upper;
	int i, j, * vet_apt, total, tem, menor;
	aha_struc * aha;
	trv_struc * esse, base1, base_invariante;
	trv_interval_list * sai, *apt_list, *apt_list2, * sai2;
	int tam_sai;
	...
	apt_list = NULL; sai = NULL; apt_list2 = NULL; sai2 = NULL; tam_sai = 0;
...
//	upper_bound_by_ron (&upper, base, str, &win_ero);
...
//		tam_sai++;
	...
	//	contar1s(aha, base->total);
		
		esse = (trv_struc *) xmalloc (sizeof (trv_struc) * base->total);
		for (i=0; i<base->total; i++) zera_basis_by_ron (&esse[i]);
		
		gera_base_invariante(&base_invariante, aha, base->total, esse);
		
//		printf("BASE invariante\n");
//		disp_basis_by_ron(&base_invariante);
		
		otimizar_aha(aha, base->total, &base_invariante);
	...
	//	}
		...
		int erase[total+1]; //apagar todas as instancias que não são lower bounds (1) erase (0) deixa WARNING SEGMENTATION FAULT (TOO MANY SOLUTIONS!!)
		erase[0] = 0;
...
//					printf("%d\n", id);
					if (!erase[id])
						tem = igual_basis_by_ron (&apt_list->interval.base_lower, &base1);
					if (!tem && !erase[id]) { //são ver para os lower bounds e não para o upper bound
						tem1 = verifica_bases_menor(&apt_list->interval.base_lower, &base1);
//						printf("tem1 %d\n", tem1);
						if (tem1) { tem = 1; break; } // então a base contem esse elemento
						tem2 = verifica_bases_menor(&base1, &apt_list->interval.base_lower);
//						printf("tem2 %d\n", tem2);
						if (tem2) {
							erase[id] = 1;
//							printf("erase = 1 %d\n", id);
...
//					disp_basis_by_ron(&base1);
					...
					apt_list = (trv_interval_list *) xmalloc (sizeof (trv_interval_list));
					copy_basis_by_ron (&base1, &apt_list->interval.base_lower);
					copy_basis_by_ron (upper, &apt_list->interval.base_upper);
					
					apt_list->interval.igual = igual_basis_by_ron (&apt_list->interval.base_upper, &apt_list->interval.base_lower);
					apt_list->prox = sai;
					sai = apt_list;
					erase[tam_sai] = 0;
//					printf("erase = 0 %d\n", tam_sai);
...
//				}
	...
	//	fclose (fd);
		...
		id = tam_sai-1;
		int cnt = 0, cnt2 = 0;
		for (apt_list = sai; apt_list; apt_list = apt_list->prox) {
			if (!erase[id]) {
				apt_list2 = (trv_interval_list *) xmalloc (sizeof (trv_interval_list));
				copy_basis_by_ron (&apt_list->interval.base_lower, &apt_list2->interval.base_lower);
				copy_basis_by_ron (&apt_list->interval.base_upper, &apt_list2->interval.base_upper);
				apt_list2->interval.igual = apt_list->interval.igual;
				apt_list2->prox = sai2;
				sai2 = apt_list2;
				cnt++;
//				disp_basis_by_ron(&apt_list2->interval.base_upper);
//				disp_basis_by_ron(&apt_list2->interval.base_lower);
//				printf("Igual = %d\n", apt_list2->interval.igual);
			}
			free_basis_by_ron (&apt_list->interval.base_lower);
			free_basis_by_ron (&apt_list->interval.base_upper);
			free(&apt_list->interval);
			id--;
			cnt2++;
		}
...
/*=*/ int search_increasing_all (trv_interval * interval, output_search_interval * * sai) {
	trv_struc minimo, upper, base_Y, base0;
	win_struc S, C;
	int * comb, num_comb, tam_idx, *winidx, i, j, k, tem;
	int section, nshifts, tam, invariante, lado;
	mat_str str;
	win_struc_list * Lista_C, * apt_Lista_C;
	int tam_Lista_C;
	output_search_interval * apt_sai;
	trv_interval_list * solucao;
...
//	printf("UPPER\n");
//	disp_basis_by_ron (&interval->base_upper);
//	printf("LOWER\n");
//	disp_basis_by_ron (&interval->base_lower);
	
//	printf("Minimo\n");
//	disp_basis_by_ron (&minimo);
	
	if (interval->igual) { //upper and lower are the same
		
		zera_basis_by_ron (&minimo);
		zera_win_by_ron (&S);
		
		minimo_lado_esquerdo (&minimo, &interval->base_upper);
		esse_min_lado_esquerdo_min (&S, &minimo, &interval->base_upper);
	...
	//	disp_win_by_ron (&S);
	...
	else { //upper and lower differents
		...
		C.hsize = 3;
		C.wsize = 3;
		C.data  = (int *) xmalloc (sizeof (int)*tam);
		for (j=0; j<tam; j++) C.data[j] = 0;
		
		tam_idx = 9; // não quer dizer que seja de 3x3 porque seja 9
		...
		for (i=tam_idx; i>=2; i--) {
			...
			for (j=0; j<num_comb; j++) {
				...
				for (k=0; k<i; k++) {
					section = comb[k] / NB;
					nshifts = comb[k] % NB;
					if (comb[k] == 0 || comb[k] == 1 || comb[k] == 2) up = 1;
					if (comb[k] == 0 || comb[k] == 3 || comb[k] == 6) left = 1;
					C.data[section] |= mask[nshifts];
					(C.size)++;
				}
				if (!up || !left) continue;
...
//		}
		...
		lado = menor_lado_possivel_da_janela (&(interval->base_upper.win));
		transform_to_small_window (&(interval->base_upper.win), lado);
	
		/* transpondo o elemento struturante */
		str_rot.dat = NULL;
		lvrotstr (&str, 4, 0, &str_rot);
		copy_win (&(interval->base_upper.win), &win_in);
		...
		/* construindo a janela $W=W'\ominus C^t$ W =W' ⊖ Ct.*/
		erode_win (&interval->base_upper.win, &win_ero, &str_rot);
		
		/* calcula o upper bound */
		zera_basis_by_ron (&upper);
		upper_bound_by_ron (&upper, &interval->base_upper, &str, &win_ero);
		...
		if (upper.total) {
			...
			/* transfere base_Y para a janela W' */
			transfer_basis_according_window (&base_Y, &interval->base_lower.win);
			
	//		printf("Upper bound para C\n");
	//		disp_basis_by_ron(&upper);
	//		printf("Dilated\n");
	//		disp_basis_by_ron(&base_Y);
//			printf("Lower\n");
...
//			disp_basis_by_ron(&interval->base_upper);
	...
	//		printf("Verifica menor = %d\n", verifica_bases_menor(&interval->base_upper, &interval->base_lower));
	//		
	//		zera_basis_by_ron(&base0);
	//		free_basis_by_ron (&base0);
	//		basis_dual_operator (&interval->base_upper, &base0);
	//		printf("Dual base UPPER");
	//		disp_basis_by_ron(&base0);
	//		
	//		zera_basis_by_ron(&base0);
	//		free_basis_by_ron (&base0);
	//		basis_dual_operator (&interval->base_lower, &base0);
	//		printf("Dual base Lower");
	//		disp_basis_by_ron(&base0);
			
			
			if (verifica_bases_menor(&interval->base_lower, &base_Y)) {
	//			printf("ENTRO\n");
				testa_equacao(&solucao, &interval->base_lower, &str, &upper);
				apt_sai = (output_search_interval *) xmalloc (sizeof(output_search_interval));
				copy_win (&apt_Lista_C->win, &apt_sai->C);
				apt_sai->list_interval = solucao;
				apt_sai->prox = * sai;
				* sai = apt_sai;
			}
		}
//		else { printf("NO ENTRO\n"); }
		if (&apt_Lista_C->win) free_win_by_ron (&apt_Lista_C->win);
		if (apt_Lista_C) free (apt_Lista_C);
	}
...
/*=*/ int paralelo_para_sequencial (trv_interval * intervalo) { //fix origem não está bem feito
	output_search_interval * sai1, * apt_sai, * sai2, * sai;
	trv_interval_list * solucao, * apt_sol;
	int i, lado, dil, ori, cnt;
	trv_struc base1, base2;
	win_struc win_ori;
	trv_interval * novo_intervalo;
...
//	disp_win_by_ron(&intervalo->base_upper.win);
	lado = menor_lado_possivel_da_janela (&intervalo->base_upper.win);
	transform_to_small_window (&intervalo->base_upper.win, lado);
//	disp_win_by_ron(&intervalo->base_upper.win);
	lado = menor_lado_possivel_da_janela (&intervalo->base_lower.win);
	transform_to_small_window (&intervalo->base_lower.win, lado);

//	disp_basis_by_ron (&intervalo->base_upper);
//	disp_basis_by_ron (&intervalo->base_lower);
	if (base_identidade (&intervalo->base_lower)) {
	...
	if (intervalo->base_lower.total == 1 && intervalo->base_lower.win.size == 1) { //entra aqui antes que o if (base_identidade (base)) {
		pilha[topo].dil = 1;
		copy_win (intervalo->base_lower.win, &pilha[topo++].C);
		free_basis_by_ron (&base1);
		copy_basis_by_ron (&intervalo->base_lower, &base1);
		free_win_by_ron (&win_ori);
		copy_win (&intervalo->base_lower.win, &win_ori);
		...
		free_win_by_ron (&win_ori);
		/* criar intervalo */
		novo_intervalo = (trv_interval *) xmalloc (sizeof (trv_interval));
		copy_basis_by_ron (&base1, &novo_intervalo->base_lower);
		copy_basis_by_ron (&base1, &novo_intervalo->base_upper);
		novo_intervalo->igual = 1;
		paralelo_para_sequencial (novo_intervalo);
		free_basis_by_ron (&base1);
	...
	if (intervalo->base_lower.total == 1) { //entran aqui os intervalos da forma 00[0]10 onde a janela não fico boa
		cnt = 0;
		for (i=0; i<intervalo->base_lower.nb; i++) {
			cnt += __builtin_popcount(intervalo->base_lower.A[i]);
		}
		if (cnt == 1) {
			diminui_base_tamanho(&intervalo->base_lower);
			pilha[topo].dil = 1;
			copy_win (intervalo->base_lower.win, &pilha[topo++].C);
			free_basis_by_ron (&base1);
			copy_basis_by_ron (&intervalo->base_lower, &base1);
	...
	search_increasing_all (intervalo, &sai1);
	...
	free_basis_by_ron (&base1);
	basis_dual_operator (&intervalo->base_upper, &base1);
	if (!intervalo->igual) {
		free_basis_by_ron (&base2);
		basis_dual_operator (&intervalo->base_lower, &base2);
	}
	
	/* criar intervalo */
	novo_intervalo = (trv_interval *) xmalloc (sizeof (trv_interval));
	if (!intervalo->igual)
		copy_basis_by_ron (&base2, &novo_intervalo->base_upper);
	else
		copy_basis_by_ron (&base1, &novo_intervalo->base_upper);
	copy_basis_by_ron (&base1, &novo_intervalo->base_lower);
	novo_intervalo->igual = intervalo->igual;
	...
	search_increasing_all (novo_intervalo, &sai2);
		...
		while (sai) {
			...
			while (solucao) {
				...
				if (dil) {
					...
					copy_basis_by_ron (&apt_sol->interval.base_upper, &base1);
					copy_basis_by_ron (&apt_sol->interval.base_lower, &base2);
				}
				else {
					if (beatriz)  printf ("Erosao\n");
					basis_dual_operator (&apt_sol->interval.base_lower, &base1);
					if (!apt_sol->interval.igual)
						basis_dual_operator (&apt_sol->interval.base_upper, &base2);
					else 
						copy_basis_by_ron (&base1, &base2);
				}
				...
				novo_intervalo = (trv_interval *) xmalloc (sizeof (trv_interval));
				copy_basis_by_ron (&base1, &novo_intervalo->base_upper);
				copy_basis_by_ron (&base2, &novo_intervalo->base_lower);
				novo_intervalo->igual = apt_sol->interval.igual;
				...
				free_basis_by_ron (&apt_sol->interval.base_lower);
				free_basis_by_ron (&apt_sol->interval.base_upper);
				free (&apt_sol->interval);
				free_basis_by_ron (&base1);
				free_basis_by_ron (&base2);
			}
