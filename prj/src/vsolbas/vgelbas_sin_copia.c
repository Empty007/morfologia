...
/* Nota: intervalo deve conter somente um intervalo */
/* verifica se o unico intervalo que esta' em intervalo ja' aparece em base */
/*=*/ int verifica_se_a_base_ja_tem_aha (aha_struc * base, trv_struc * intervalo) {
	int ind, i, j, tem=0, k;
	...
	for (i=0, tem=0; i<base->total && !tem; i++) {
		for (k = 0; k < base->intervs[i].total; k++) {
			ind = base->intervs[i].ind[k];
			for (j=0, tem=1; j<base->nv && tem; j++) {
				if ( ((base->intervs[i].A[ind+j/NB] & mask[j%NB])  != 
					  (intervalo->A[j/NB] & mask[j%NB])) || 
					((base->intervs[i].B[ind+j/NB] & mask[j%NB])  != 
					 (intervalo->B[j/NB] & mask[j%NB]))) {
						tem=0;
					}
			}
...
/*=*/ int base_ele_erre_W_by_edu (trv_struc * sai, trv_struc * base, mat_str * str, trv_struc * upper, aha_struc * aha, int indice) {
	/* trv_struc * base represents only one interval */
	win_struc * win = &(base->win);
	mat_str str_rot, * ker;
	win_struc * win_ero, win_in;
	int i, * X_ker, * Y_ker, size, m, n, points, jatem, j;
	trv_struc base_aux;
		...
		if (base_aux.total == 1) {
			...
			if (!retorno) {
				...
				if (retorno2) {
					...
					jatem = 0;
					for (j = 0; j < indice; j++) {
						if (verifica_se_a_base_ja_tem_aha (&aha[j], &base_aux)) {
							jatem = 1;
							break;
						}
					}
...
/*=*/ int aha_crescente_W_by_edu (aha_struc * sai, trv_struc * base, mat_str * str, trv_struc * upper, aha_struc * aha, int indice) {
	trv_struc elle;
	int i, ii, tam, ind1;
...
//	printf("aha!! %d\n", aha[2].total);
//	printf("aha!! %d\n", aha[3].total);
	base_ele_erre_W_by_edu (&elle, base, str, upper, aha, indice);
...
//	printf("%d\n", aha[3].total);
	...
	for (i=0; i<base->total; i++) {
		...
		aha_crescente_W_by_edu (&aha[i], &quebrada[i], str, &upper, aha, i);
