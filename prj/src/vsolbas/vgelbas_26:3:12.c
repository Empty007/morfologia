...
int optimizar_aha(aha_struc * aha, 
				  int tam_aha) {
	...
	for (j=0; j<tam_aha; j++) { /* para cada conjunto H */
		if (aha[j].total >= 2) {
			int working[9] = {0, 0, 0, 0, 0, 0, 0, 0, 0 };
			ok = 1;
			for (k = 0; k < aha[j].total; k++) if (!working[k]) {
				for (i = 0; i < tam_aha; i++) {
					if (aha[i].total == 1) {
						bo = 1;
						for (jj=0; jj<aha[j].nb; jj++) {
...
/*=*/ int testa_equacao (trv_struc_list **s, trv_struc * base, 
	...
	trv_struc_list * sai, *apt_list, *apt_list2, * sai2;
	int tam_sai;
...
//	}
	...
	int erase[total+1]; //apagar todas as instancias que não são lower bounds (1) erase (0) deixa
	erase[0] = 0;
...
//					printf("%d\n", id);
					...
					if (!tem && !erase[id] && id != 0) { //são ver para os lower bounds e não para o upper bound
						tem1 = verifica_bases_menor(&apt_list->base, &base1);
//						printf("tem1 %d\n", tem1);
						if (tem1) { tem = 1; break; } // então a base contem esse elemento
						tem2 = verifica_bases_menor(&base1, &apt_list->base);
//						printf("tem2 %d\n", tem2);
						if (tem2) {
							erase[id] = 1;
//							printf("erase = 1 %d\n", id);
...
//					printf("erase = 0 %d\n", tam_sai);
	...
	for (apt_list = sai; apt_list; apt_list = apt_list->prox) {
		if (!erase[id]) {
			apt_list2 = (trv_struc_list *) xmalloc (sizeof (trv_struc_list));
			copy_basis_by_ron (&apt_list->base, &apt_list2->base);
			disp_basis_by_ron(&apt_list2->base);
			apt_list2->prox = sai2;
			sai2 = apt_list2;
			cnt++;
		}
