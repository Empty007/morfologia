...
int evalua(trv_struc * base, win_struc * C, str_A_star * A_star, int g_father, int erosion) {
	trv_struc base1;
	win_struc S;
...
int evalua_A_star (output_search * sai_in, int g_father, int erosion) { // g_actual é o C anterior
	...
	while (sai) {
		...
		while (solucao) {
			apt_sol = solucao;
			solucao = solucao->prox;
			A_star_list = (str_A_star_list *) node_allocation (sizeof(str_A_star_list));
...
/*=*/ int paralelo_para_sequencial (trv_struc * base, str_A_star * node) {
	...
	if (base->total == 1 && base->win.size == 1) { //entra aqui antes que o if (base_identidade (base)) {
		...
		free_win_by_ron (&win_ori);
		paralelo_para_sequencial (&base1, NULL); // no se si es cierto
		free_basis_by_ron (&base1);
