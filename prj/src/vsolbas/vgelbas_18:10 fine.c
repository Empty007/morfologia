...
int disp_basis_by_ron_2 (trv_struc * basis) {
	...
	B = (int *) xmalloc(sizeof(int)*basis->nb);
...
/*=*/ int testa_base_ele_erre_W (void) { 
	...
	if (beatriz) disp_win_by_ron (&entra.win);
    /* construindo a base */
...
/*=*/ int gera_lower_bounds_crescente_by_edu (trv_struc_list **s, trv_struc * base, 
                                       mat_str * str) {
	trv_struc * quebrada;
	trv_struc upper;
	int i, j, * vet_apt, total, tem;
	...
	if (Z) {
		...
		if (igual) {
			...
			if (!tem) {
				...
				xmalloc (sizeof(trv_struc_list));
				zera_basis_by_ron (&apt_sol->base);
				copy_basis_by_ron (&upper, &apt_sol->base);
				apt_sol->prox = sol;
				sol = apt_sol; cont_sol++;
			}
		}
		
//		printf("ENTRO por fas q sean 2\n");
//		disp_win_by_ron(str);
...
/*=*/ int search_increasing_all (trv_struc * base, output_search * * sai) {
	...
	*sai = NULL;
	...
	while (Lista_C) {
		...
		gera_lower_bounds_crescente_by_edu (&lower, base, &str);
