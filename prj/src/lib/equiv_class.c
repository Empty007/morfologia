/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
 ...
 >>>>
 >>>> Date Last Updated: mayo 26 2011
 >>>>
...
equivalence_struct *equival;
{
	...
	int translate[nroIntervalos][tamanhoWin][2]; /* vetor de todas as translacoes [nroIntervalo real de base][nro de Translacao][y, x]*/ 
	int nrovars[nroIntervalos]; /*nro de 1's*/
	int nroTranslations[nroIntervalos][3]; /*nro de translacoes de cada elemento da base, indice para ordenar, nro de 1's de cada intervalo*/
    ...
    for (i = 0; i < nroIntervalos; i++) if (!vis[i]) {
		...
		int minx = 0;
//		printf("Track %d\n", i);
        for (j = i + 1; j < nroIntervalos; j++) if (!vis[j]) {
            ...
            if (bo) {
                vis[j] = 1;
//				printf("dy = %d, dx = %d, i = %d, j = %d\n", dify, difx, i, j);
                translate[nroNovosIntervalos][cnt][0] = dify;
                translate[nroNovosIntervalos][cnt][1] = difx;
				miny = dify < miny ? dify : miny;
				minx = difx < minx ? difx : minx;
                cnt++;	
            }
        }
		//ordenar translacoes para que nao fiquem menores que 0
		if (miny != 0 || minx != 0) {
			for (j = 0; j < cnt; j++) {
				translate[nroNovosIntervalos][j][0] += -miny;
				translate[nroNovosIntervalos][j][1] += -minx;
			}
		}
		...
		nroTranslations[nroNovosIntervalos][2] = tamanhos[i];
        nroNovosIntervalos++;
    }
	
	qsort(nroTranslations, nroNovosIntervalos, 3 * sizeof(int), comp2);
