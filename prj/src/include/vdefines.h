 /*
  * Khoros: $Id: vdefines.h,v 1.5 1992/03/20 22:34:45 dkhoros Exp $
  */

 /*
  * $Log: vdefines.h,v $
 * Revision 1.5  1992/03/20  22:34:45  dkhoros
 * VirtualPatch5
 *
  */ 
...
/* >>>>>>>>>>>>>>>>>>>>>>>>>>>>> <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
   ...
   >>>>								 <<<<
   >>>>			     Added check to VStrcpy to not copy  <<<<
   >>>>			     a NULL string, but to return NULL   <<<<
   >>>>						Mark - 9/3/91    <<<<
   >>>>                                                          <<<<
   >>>>			     Added k-alloc routines in order     <<<<
   >>>>			     to allocate memory and make sure    <<<<
   >>>>			     that an alloc() of 0 bytes is not   <<<<
   >>>>			     performed.				 <<<<
   >>>>						Mark - 11/7/91   <<<<
   >>>>                                                          <<<<
   >>>>                      Added VStrncpy                      <<<<
   >>>>                      Added VStrncmp                      <<<<
   >>>>                      Added VStr3cat                      <<<<
   >>>>                      Changed k-realloc to malloc on NULL <<<<
   >>>>                      input strings                       <<<<
   >>>>                                        Steve J - 12/2/91 <<<<
   >>>>                                                          <<<<
   >>>>>>>>>>>>>>>>>>>>>>>>>>>>> <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< */
