...
extern int num_sol;
extern int imprimeZ;
int imprime_solucao = 0;
int imprime_crescente = 0;
int imprime_crescente_elem;
int leZ = 0;
int menor_complexidade = 0;
int menor_topo = 0;
...
/*=*/ int ori_esta_win_by_ron (win_struc * win) {

  int i, j;
  int  nshifts, mask, section, data;

  if (win == NULL) {
    fprintf (stderr, "ori_win_by_ron () pointer to NULL\n");
    return 0;
  }
...
/*=*/ int paralelo_para_sequencial_direto (trv_struc * base) {
  ...
  win_struc win_ori;
  int conta_pontos;
  int complexidade, contador, unioes;
  ...
  if (base_identidade (base)) {
    ...
    if (!imprime_solucao) {
      for (i=0; i<topo; i++) {
        ...
        contador = function_size_win (&pilha[i].C);
        if (contador == 1) contador = 0;
        else {
          if (ori_esta_win_by_ron (&pilha[i].C)) {contador--;}
          if (!ori_esta_win_by_ron (&pilha[i].C)) {contador += unioes - 1;}
          else {contador += unioes;}
          /* contador igual ao numero de shifts e unioes */
          
        }
        complexidade += contador;
        printf ("EE %03d = %d\n", i, contador);
      }
    }
    else {
    ...
    if (menor_complexidade == 0) {
      menor_complexidade = complexidade;
      menor_topo = topo;
    }
    else {
      if (menor_complexidade > complexidade) {
        menor_complexidade = complexidade;
        menor_topo = topo;
      }
    }
    printf ("Solucao com %d EE's\n", topo);
    printf ("Fim Solucao %03d Complexidade = %d ===============================\n", num_sol, complexidade);
    return 1;
  }
  ...
  if (base->total == 1 && conta_pontos == 1) {
    ...
    if (imprime_crescente_elem) printf ("Empilhou: topo = %d\n", topo);
    if (imprime_crescente_elem) printf ("Elem. Estruturante:\n");
    if (imprime_crescente_elem) disp_win_by_ron (&pilha[topo-1].C);
    if (imprime_crescente_elem) printf ("\n");
...
/*=*/ int lvsolbas_crescente_paralelo_sequencial (char * fname1) {
  ...
  printf ("A Base de Entrada tem %d Intervalos\n", basis1.total);
...
/*=*/ int main (int argc, char * * argv) {
  beatriz = 0;
  topo = 0;
  // imprime_crescente_elem = 1;
  imprime_crescente_elem = 0;
  imprime_crescente = 0;
  imprimeZ = 0;
  leZ = 0;
  num_sol = 0;
  printf ("Paralelo para Sequencial versao 1.1 09/Maio/2000\n");
  if (argc == 1) {
    return 0;
  }

  imprime_solucao = atoi (argv[2]);

  if (!lvsolbas_crescente_paralelo_sequencial (argv[1])) {
    fprintf(stderr, "vlsol : lvsolbas() failed\n");
    return 1;
  }
  printf ("Tivemos %d solucoes\n", num_sol);
  if (num_sol) {
    printf ("Menor Complexidade = %d", menor_complexidade);
    printf ("com %d EEs\n", menor_topo);
  }
  return 0;
}
