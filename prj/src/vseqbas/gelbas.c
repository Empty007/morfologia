...
int disp_basis_by_ron_2 (trv_struc * basis) {
...
/*=*/ int upper_bound_by_ron (trv_struc * base_out, trv_struc * base_in, 
  ...
  // printf ("Antes erosion\n");
  erosion_basis (base_in, str, base_out);
  // printf ("Depois erosion\n");
...
/*=*/ int testa_gera_lower_bounds_00 (void) {
  ...
  /* construindo uma base com um elemento */
    ...
    /* construindo a base */
       ...
       aha = (aha_struc *) node_allocation (sizeof (aha_struc) * entra.total);
...
/*=*/ int testa_gera_lower_bounds (void) {
  ...
  /* contruindo o elemento estruturante para a dilatacao*/
    ...
    str_dil.dat = (short *) node_allocation (sizeof(short)*str.m*str.n);
...
/*=*/ int testa_gera_lower_bounds_bom (trv_struc * ent, mat_str * ker) {
  ...
  quebrada = (trv_struc *) node_allocation (sizeof (trv_struc) * entra.total);
...
/*=*/ int gera_lower_bounds_crescente (trv_struc_list **s, trv_struc * base, 
  ...
  int tam_sai;
  FILE * fd;
  unsigned int total, i, j, * vet_apt;
  int * num, * mta, m, mm;
  ...
  /* cada casa de vet_apt[j] guarda um indice de um intervalo de aha[j] */
  vet_apt = (int *) node_allocation (sizeof (int) * base->total);
  num = (int *) node_allocation (sizeof (int) * base->total);
  mta = (int *) node_allocation (sizeof (int) * base->total);
  m = 0;
  for (i=0, total=1; i<base->total; i++) {
    vet_apt[i] = 0;
    if (aha[i].total) {
  ...
  // printf ("total de possibilidades = %u\n", total);
  ...
  i = 0;
  while (mta[0] < num[0]) { /* num. total de prod. cartesiano */
    i++;
    // printf ("%u/%u                                   \r", i, total);
    if (beatriz) {
      // printf ("%s\r", " ");
      printf ("%u/%u\n", i, total);
    }
    ...
    mta[m-1]++;
    for (mm=m-1; mm>0; mm--) {
      if (mta[mm]==num[mm]) {mta[mm]=0;mta[mm-1]++;}
    }
  }
  ...
  *s = sai;
  ...
  while (sai) {
    ...
    if (imprimeZ) {
      // printf ("Basis %d:\n", ++i);
      // disp_basis_by_ron (&apt_list->base);
      if (!verifica_lado_direito_igual (&apt_list->base)) {
        printf ("Erro\n");
      }
      append_basis_by_ron ("Z.txt", &apt_list->base);
    }
...
/*=*/ int verifica_lado_direito_igual (trv_struc *base) {
  int i, j, ind;

  for (i=1; i<base->total; i++) {
    ind = base->ind[i];
    for (j=0; j<base->nb; j++) {
      if (base->B[j+0] != base->B[j+ind])
        return 0;
    }
  }
  return 1;
...
/*=*/ int testa_equacao_dilatacao_crescente (trv_struc * base, mat_str * str, 
  ...
  if (Z) {
    ...
    /* calcula o upper bound */
    zera_basis_by_ron (&upper);
    // printf ("Entrada para Calculo do Upper: %d elementos\n", base->total);
    upper_bound_by_ron (&upper, base, str);
    lado = menor_lado_possivel_da_janela (&(upper.win));
    transform_to_small_window (&(upper.win), lado);
    // printf ("Depois Upper\n");
    ...
    for (apt_Z = Z; apt_Z; apt_Z = apt_Z->prox) {
      ...
      copy_basis_by_ron (&(apt_Z->base), &base_Z);
      // printf ("Lower Bound %d: %d elementos\n", cont_Z, base_Z.total);
      // transfer_basis_according_window (&base_Z, &W_duas_linhas);
      /* aqui base_Z esta' definido em W'' */
      retorno = verifica_bases_menor (&base_Z, &upper);
      // printf ("retorno = %d\n", retorno);
      ...
      if (igual) {
        ...
        if (!tem) {
          // printf ("Base Incluida\n");
          // disp_basis_by_ron (&base_X);
          apt_sol = (trv_struc_list *) 
                     node_allocation (sizeof(trv_struc_list));
          zera_basis_by_ron (&apt_sol->base);
          copy_basis_by_ron (&base_X, &apt_sol->base);
      ...
      if (desmonta.total > 30) {
        printf ("Sai pois desmonta.total = %d\n", desmonta.total);
        // printf ("Lower:\n");
        // disp_basis_by_ron (&base_Z);
        // printf ("Upper:\n");
        // disp_basis_by_ron (&upper);
      ...
      for (j=1; j<=desmonta.total; j++) {
        comb = NULL; num_comb = numero_de_combinacoes (desmonta.total, j);
        for (k=0; k<num_comb; k++) {
          // printf ("(%d, %d) - k = %d/%d sol = %d\r", desmonta.total, j, k+1,
          // num_comb, cont_sol);
          if (beatriz) printf ("(%d, %d) - k = %d/%d\n", desmonta.total, j, k+1, num_comb);
          if (beatriz) {
            if (k==num_comb-1) {
          ...
          for (kk=0; kk<j; kk++) {
            ...
            for (rr=0; rr<base_D.nb; rr++) {
              ...
              base_D.B[ind2+rr] = (~desmonta.A[ind1+rr]);
              // base crescente
              // base_D.B[ind2+rr] = upper.B[0+rr];
            }
          }
          free_basis_by_ron (&base_E);
          copy_basis_by_ron (&base_Z, &base_E);
          // if (!verifica_lado_direito_igual (&base_Z)) printf ("\nErro 1\n\n");
          free_basis_by_ron (&base_X);
          // if (!verifica_lado_direito_igual (&base_D)) printf ("\nErro 2\n\n");
          union_basis_by_ron (&base_E, &base_D, &base_X);
          free_basis_by_ron (&base_D);
          free_basis_by_ron (&base_E);
          /* aqui nos temos uma possivel solucao base_X */
          ...
          if (!verifica_lado_direito_igual (&base_X)) {
            /* base_X nao e' crescente */
            free_basis_by_ron (&base_X);
            continue;
          }
...
/*=*/ int testa_gera_lower_bounds_bom_crescente (trv_struc * ent, 
                                                 mat_str * ker) {
  trv_struc entra, sai, testa;
  mat_str str, str_dil;
  int lado;
  aha_struc * aha;
  trv_struc * quebrada;
  trv_struc_list * lower, * solucao, * apt_sol;
  ...
  // printf ("AHA\n");
  for (i=0; i<entra.total; i++) {
    aha_crescente_W (&aha[i], &quebrada[i], ker);
    // printf ("AHA i = %02d => %03d Pairs\n", i+1, aha[i].total);
    if (aha[i].total) {
      lado = menor_lado_possivel_da_janela (&(aha[i].win));
      transform_to_small_window (&(aha[i].win), lado);
  ...
  gera_lower_bounds_crescente (&lower, &entra, ker);
  testa_equacao_dilatacao_crescente (&entra, ker, lower, &solucao);
...
/*=*/ int minimo_lado_esquerdo (trv_struc * sai, trv_struc * base) {
  ...
  index = (int *) node_allocation (sizeof (int) * base->total);
  tam_index = 0;
  for (i=0; i<base->total; i++) {
    ind = base->ind[i]; cont = 0;
    for (j=0; j<base->nv; j++) {
  ...
  sai->nv = base->nv;
  sai->nb = base->nb;
  sai->A = (int *) node_allocation (sizeof(int) * sai->total * sai->nb);
  sai->B = (int *) node_allocation (sizeof(int) * sai->total * sai->nb);
  sai->ind = (int *) node_allocation (sizeof(int) * sai->total);
...
/*=*/ int esse_min_lado_esquerdo_min (win_struc * conj, trv_struc * min,
  ...
  win_vec = (win_struc*) node_allocation (sizeof(win_struc) * min->total);
  ...
  for (i=0; i<min->total; i++) {
    ...
    win_vec[i].data  = (int *) node_allocation (sizeof (int)*tam);
  ...
  win_vec_str = (win_struc*) node_allocation (sizeof(win_struc) * base->total);
  str_vec     = (mat_str*) node_allocation (sizeof(mat_str) * base->total);
  ...
  for (i=0; i<base->total; i++) {
    ...
    win_vec_str[i].data  = (int *) node_allocation (sizeof (int)*tam);
    str_vec[i].dat = (short *) node_allocation (sizeof (short)*tam_str);
...
/*=*/ int window_to_str (win_struc * win, mat_str * str) {
  ...
  str->p = 1;
  str->tag = MM_IMG;
  str->dat = (short *) node_allocation (sizeof (short)*tam);
  winidx = NULL;
  win_index (win, &winidx);
...
/*=*/ int conta_pontos_da_extremidade_esquerda (trv_struc * base) {
...
int base_identidade (trv_struc * base) {
  ...
  if (base->total != 1 || conta_pontos != 1) return 0;
